                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <div class="flex">
                          <img class="w-7 h-7 rounded-full" :src="run.driver_info.user.avatar" >
                          <span class="pl-2 pt-1">
                            {{ run.driver_info.user.name }}
                          </span>
                        </div>
                        <span class="pl-9 text-sm text-gray-400">{{ run.driver_info.car_brand }}</span>
                      </td>
                      <td
                      ...
                      <td
                          class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"
                      >
                        <BadgeBoolean :text="run.ended_at === null ? 'Em andamento' : 'Encerrada'" :value="run.deleted_at === null ? true : false" />
                      </td>
                      <td
                          class="whitespace-nowrap px-3 py-4 text-sm text-gray-500"
                      >
                      ...
                      >
                        <Link
                            :href="
                            route('runs.edit', run.id)"
                            class="text-indigo-600 hover:text-indigo-900"
                        >
                          Edit
                        </Link>
                        <button
                            @click="deleteUser(run.id)"
...
let usersUrl = computed(() => {
  const url = new URL(route("runs.index"));
...
const deleteUser = (id) => {
  if (confirm("Are you sure you want to delete this student?")) {
    deleteForm.delete(route("runs.destroy", id), {
      preserveScroll: true,
    });
  }
};
